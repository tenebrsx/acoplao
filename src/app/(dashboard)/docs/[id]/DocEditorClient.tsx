...
export function DocEditorClient({ doc, isPublicView = false }: { doc: any, isPublicView?: boolean }) {
  ...
  const copyShareLink = () => {
    const url = `${window.location.origin}/shared-docs/shared/${doc.share_token}`
    ...
    <div className="animate-in delay-100" style={{ maxWidth: '860px', margin: '0 auto' }}>
      ...
      {!isPublicView && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          <Link href="/docs" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', color: 'var(--text-tertiary)', fontSize: '0.875rem', textDecoration: 'none' }}>
