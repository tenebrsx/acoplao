...
export function DocEditorClient({ docId, initialDoc, isPublicView = false }: { docId: string, initialDoc: any, isPublicView?: boolean }) {
  ...
  const copyShareLink = () => {
    const url = `${window.location.origin}/shared-docs/shared/${shareToken}`
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 100px)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flex: 1 }}>
          {!isPublicView && (
            <Link href="/docs" style={{ color: 'var(--text-tertiary)', textDecoration: 'none', display: 'flex', alignItems: 'center' }}>
              <ArrowLeft size={20} />
            </Link>
          )}
          <input
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <input 
                          type="text" 
                          readOnly 
                          value={`${window.location.origin}/shared-docs/shared/${shareToken}`} 
